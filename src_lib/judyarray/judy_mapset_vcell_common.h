...
template <
	...
	typename TAllocator,
	typename TTraits>
class __judy_mapset_vcell_base : private TTraits
{
private:
...
public:
	...
	typedef __judy_mapset_vcell_base <TKey, TValue, THashFunc, TEqualFunc, TAllocator, TTraits> __this_type;
...
private:
	...
	Word_t               m_magic_empty_elt;

	typedef std::map <key_type, mapped_type> second_level_storage_type;
	second_level_storage_type m_2level_storage;
...
//
public:
	__judy_mapset_vcell_base ()
	{
		m_judy = 0;
		m_size = 0;
		m_magic_empty_elt = 0xDEADBEAF;
	}
	...
	__judy_mapset_vcell_base (const __this_type& a)
		:
		m_judy (0),
		m_size (0),
	...
	void clear ()
	{
		...
		m_debug_info.m_list_count       = 0;
		m_debug_info.m_list_item_count  = 0;

		m_2level_storage.clear ();
		::JudyLFreeArray (&m_judy, 0);
		*/
	}

	__judy_mapset_vcell_base& operator = (const __this_type& a)
	{
	...
	void swap (__this_type& a)
	{
		...
		std::swap (m_2level_storage, a.m_2level_storage);
	}
...
private:
	...
	public:
		...
		bool                   m_end;
		bool                   m_inside_2l_storage;

		typename second_level_storage_type::iterator m_2l_storage_it;
		...
		iterator_base (const __this_type *obj, Word_t index, Word_t value)
			:
			m_obj (obj),
			m_index (index),
			m_end (false)
		...
		iterator_base (const __this_type *obj,
			           typename second_level_storage_type:iterator it)
			:
			m_obj (obj),
			m_index (0),
			m_end (false),
		...
		iterator_base (const __this_type *obj)
		{
			...
			PWord_t p = (PWord_t) JudyLFirst (m_obj -> m_judy, &m_index, 0);
			if (p){
				m_value.m_judy_int = *p;
				m_inside_list = true;
				m_2level_storage_it = obj -> m_2l_storage.begin ();
			}else{
				m_end = true;
			}
		}
...
public:
	...
	private:
		...
		friend class __judy_mapset_vcell_base;
